...
func TestAccKubernetesCluster_addonProfileConfcom(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_kubernetes_cluster", "test")
	r := KubernetesClusterResource{}
	...
	data.ResourceTest(t, r, []acceptance.TestStep{
		{
			// Enable Confcom
			Config: r.addonProfileConfcomConfig(data, true),
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
			),
		},
		...
		{
			// Disable Confcom
			Config: r.addonProfileConfcomConfig(data, false),
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
			),
		},
...
func (KubernetesClusterResource) addonProfileConfcomConfig(data acceptance.TestData, enabled bool) string {
	return fmt.Sprintf(`
provider "azurerm" {
  features {}
}
